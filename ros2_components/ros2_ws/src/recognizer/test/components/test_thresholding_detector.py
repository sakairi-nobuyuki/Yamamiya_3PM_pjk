...
class TestThresholdingDetector:
    ...
    def test_detect_filter(self, mock_s3_dataset: S3ImageIO, mock_s3: S3ImageIO, mock_s3_config: S3ConfigIO) -> None:
        detector = ThresholdingDetector(type="saturation", threshold_value=20, config_s3=mock_s3_config)
        img_file_name_list = [item for item in mock_s3_dataset.blob if "thresholding_train" in item]
