...
class TestThresholdingTrainer:
    def test_run(self, mock_train_config: Dict[str, Any], mock_s3_dataset: S3ImageIO, mock_s3_config: S3ConfigIO):
        trainer = ThresholdingTrainer(mock_train_config, dataset_s3=mock_s3_dataset, config_s3=mock_s3_config)

        assert isinstance(trainer, ThresholdingTrainer)

    def test_thresholding(self, mock_train_config: Dict[str, Any], mock_s3_dataset: S3ImageIO, mock_s3_config: S3ConfigIO):
        trainer = ThresholdingTrainer(mock_train_config, dataset_s3=mock_s3_dataset, config_s3=mock_s3_config)
    ...
    def test_run(self, mock_train_config: Dict[str, Any], mock_s3_dataset: S3ImageIO, mock_s3_config: S3ConfigIO):
        ...
        cropped_image_list = [item for item in mock_s3_dataset.blob if "region_extractor/object_filter_train" in item]
