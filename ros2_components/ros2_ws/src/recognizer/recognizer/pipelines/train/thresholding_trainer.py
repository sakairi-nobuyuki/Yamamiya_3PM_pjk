...
class ThresholdingTrainer:
    ...
    def __init__(
        ...
        self.thresholding_saturation_trainer = ThresholdingSaturationTrainer(**train_config, s3 = dataset_s3)
    ...
    def __create_object_filter_dataset(self, config_dict: Dict[str, Any]) -> None:
        ...
        for file_name in file_name_list:
            ...
            for bbox in bboxes:
                ...
                file_name_cropped = f"region_extractor/object_filter_train/{str(uuid4()).replace('-', '')}.png"
    ...
    def train_thresholding(self)  -> Dict[str, Any]:
        ...
        if min_loss_thresholding_hsv > min_loss_thresholding_saturation:
            ...
            res_dict = dict(type="hsv", threshold_value=int(res_list[0]), lower_green=[int(res_list[1]), int(res_list[2]), int(res_list[3])], upper_green=[int(res_list[4]), int(res_list[5]), int(res_list[6])])
        else:
