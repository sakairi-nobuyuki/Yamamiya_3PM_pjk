...
class TestIoModuleFactory:
    ...
    def test_create(self, mock_io_module_config_dict: Dict[str, str]) -> None:
        ...
        assert isinstance(
            factory.create(**dict(type="onnx", bucket_name="models")), OnnxS3
        )
        assert isinstance(
            factory.create(**dict(type="image", bucket_name="dataset")), S3ImageIO
        )
        assert isinstance(
            factory.create(**dict(type="config", bucket_name="config")), S3ConfigIO
        )
